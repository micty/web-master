...
define('WebSite/Packages', function (require, module, exports) {
    ...
    var PackageBlock = require('PackageBlock');
    var Log = require('Log');
    ...
    return {
        ...
        */
        init: function (meta) {
            ...
            var dest = meta.packages.dest;
            var dir = meta.cwd + dest.dir;      //分包资源打包后的输出目录。
            var file = meta.cwd + dest.file;    //总包配置文件的输出路径。
        ...
        */
        parse: function (meta) {
            ...
            block.on('compile', {
                'html-block': function () {
                    var args = [...arguments];
                    var values = meta.emitter.fire('package', 'compile', 'html-block', args);
                    return values.slice(-1)[0];
                },
                'js-block': function () {
                    var args = [...arguments];
                    var values = meta.emitter.fire('package', 'compile', 'js-block', args);
                    return values.slice(-1)[0];
                },
            });
        ...
        */
        build: function (meta, config) {
            ...
            //短路径补全。
            var begin = options.begin ? meta.cwd + options.begin : '';
            var end = options.end ? meta.cwd + options.end : '';

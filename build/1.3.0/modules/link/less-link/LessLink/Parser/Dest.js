...
define('LessLink/Parser/Dest', function (require, module, exports) {
    ...
    function getName(htdocs, less) {
        ...
        // `html/test/view.less`
        var name = Path.relative(htdocs, less);
    ...
    return {
        ...
        */
        get: function (options) {
            ...
            var cssDir = htdocs + options.css;     // css 目录完整路径。



            var name = getName(htdocs, file);
            var hrefDir = Path.relative(dir, cssDir) + '/';  //href 属性中使用的相对目录。

            var dest = cssDir + name;   //完整文件名。
            var href = hrefDir + name;  //相对路径。

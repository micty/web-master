        ...
        */
        minify: function (options) {
            ...
            if (error) {
                ...
                var prefix = lines[error.line - 2] || '';
                var line = lines[error.line - 1];       //这一行是发生错误的行。 因为 error.line 是从 1 开始的。
                var suffix = lines[error.line] || '';

                console.log('js 压缩错误:'.bgRed, src.red );
                console.log(JSON.stringify(error, null, 4).bgRed);  //标红。

                console.log('压缩前的内容出错的上下文:');
                console.log(((error.line - 1) + ':').cyan, prefix.bgMagenta);
                console.log(((error.line - 0) + ':').cyan, line.bgRed);
                console.log(((error.line + 1) + ':').cyan, suffix.bgMagenta);
                console.log('');

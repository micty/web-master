    ...
    WebSite.prototype = {
        ...
        */
        watch: function (done) {
            ...
            Directory.delete(packageDir);   //先清空，避免使用者意外用到。
            Directory.create(packageDir);   //这里要先创建 package 目录，否则 watcher 会出错，暂未找到根本原因。

            var packageFile = meta.packageFile;

            if (packageFile) {
                var dest = Path.join(cwd, packageFile);
                Package.write(dest); //写入一个空 {} 到总包。
            }
            ...
            //并行处理任务。
            var tasks = new Tasks([processMasters, processPackages, ]);

            tasks.on('each', function (task, index, done) {
                task ? task(done) : done();
            });
        ...
        */
        build: function (options, done) {
            ...
            tasks.on('all', function () {
                ...
                Files.clear(cwd, options.clean, '清理'); //构建后需要清理的文件或目录。
